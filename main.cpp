...
void collect_results(SSD_Device& ssd, Host_System& host, const char* output_file_path)
{
	...
	for (unsigned int stream_id = 0; stream_id < IO_flows.size(); stream_id++) {
		cout << "Flow " << IO_flows[stream_id]->ID() << " - total requests generated: " << IO_flows[stream_id]->Get_generated_request_count()
		    << " total read requests:" << IO_flows[stream_id]->Get_generated_read_request_count()
		    << " total write requests:" << IO_flows[stream_id]->Get_generated_write_request_count()
			<< " total requests serviced:" << IO_flows[stream_id]->Get_serviced_request_count()
			<< " total read_count required:" << IO_flows[stream_id]->Get_required_read_dt_count()<< endl;
		cout << "                   - device response time: " << IO_flows[stream_id]->Get_device_response_time() << " (us)"
			...
			/*<< "Get min end to end request delay()"<<IO_flows[stream_id]->Get_min_end_to_end_request_delay()
			<< "Get max end to end request delay()"<<IO_flows[stream_id]->Get_max_end_to_end_request_delay()*/
	}
}
...
int main(int argc, char* argv[])
{
    ...
    string ssd_config_file_path, workload_defs_file_path;
    string ssd_file("ssdconfig.xml");
    string workload_file("workload.xml");

    ssd_config_file_path.assign(ssd_file);
    workload_defs_file_path.assign(workload_file);

    cout<<ssd_config_file_path<<endl;
    cout<<workload_defs_file_path<<endl;
