																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		sim_time_type device_response_time = Simulator->Time() - request->Enqueue_time;
        request_delay = Simulator->Time() - request->Arrival_time;

        //PRINT_MESSAGE("request_delay"<<request_delay);
		STAT_serviced_request_count++;
		STAT_serviced_request_count_short_term++;

		STAT_serviced_DT_count+=request->size/4;

		if(STAT_serviced_DT_count>=DT_Count_limit)
        {
		    request_delay+=100*SIM_TIME_TO_MICROSECONDS_COEFF;
        }
    ...
    uint32_t IO_Flow_Base::Get_generated_read_request_count()
    {
        return STAT_generated_read_request_count;
    }

    uint32_t IO_Flow_Base::Get_generated_write_request_count()
    {
        return STAT_generated_write_request_count;
    }

	// To get the required_DT_Count
    uint32_t IO_Flow_Base::Get_required_read_dt_count()
    {
        //STAT_generated_read_DT_count=current_read_count;
