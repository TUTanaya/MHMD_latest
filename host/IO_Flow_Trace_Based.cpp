...
#include "../exec/Parameter_Set_Base.h"
#include <unistd.h>
...
Host_IO_Request *IO_Flow_Trace_Based::Generate_next_request()
{
    ...
    {
        current_trace_line[ASCIITraceTypeColumn].assign("1");
    }
	...
	request->size=stoi(current_trace_line[ASCIITraceSizeColumn]); //Need to use for DT_Count
    ...
    /*if(current_read_count > DT_Count_limit)
    {
        /*time_delay_start=Simulator->Time();
        sleep(1);
        time_delay_end=Simulator->Time();
        time_delay=time_delay_end-time_delay_start;
        time_delay=1000*SIM_TIME_TO_MICROSECONDS_COEFF;
        //PRINT_MESSAGE("Delay incorporated: "<<time_delay);

        request->Arrival_time = time_offset + time_delay + Simulator->Time();
        //PRINT_MESSAGE("Arrival time: "<< request->Arrival_time);

    }
    else*/

    request->Arrival_time = time_offset + Simulator->Time();
    //PRINT_MESSAGE("Arrival_time"<< request->Arrival_time);
...
void IO_Flow_Trace_Based::Start_simulation()
{
	...
	while (std::getline(trace_file, trace_line))
	{
		...
		//Incorporated for tracking DT: TR
		current_trace_line[ASCIITraceTypeColumn].assign("1");//To make each request as read request

		if(current_trace_line[ASCIITraceTypeColumn].compare(ASCIITraceReadCode)==0)
        {
		    test_read++;
            PRINT_MESSAGE("Current Read request: " << test_read);
        }


        int read_count=stoi(current_trace_line[ASCIITraceSizeColumn]);
        if(read_count%4==0)
            current_read_count+=read_count/4;
        else
            current_read_count+=(read_count/4) + 1;

        if(current_read_count>DT_Count_limit)
            PRINT_MESSAGE("Exceeded" );

        //PRINT_MESSAGE("Current Read Count: " << current_read_count);


		total_requests_in_file++;
        PRINT_MESSAGE("Total_request: " << total_requests_in_file);
